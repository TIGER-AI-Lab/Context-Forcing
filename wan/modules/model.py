...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_attn = WanSelfAttention(dim, num_heads, window_size, qk_norm,
                                          eps)
        if context_mix:
            self.self_attn_context = WanSelfAttention(dim, num_heads, window_size, qk_norm,
                                            eps)
    ...
    def forward(
        ...
        if mode == "base":
            ...
            y = self.self_attn_context(
                self.norm1(x) * (1 + e[1]) + e[0], seq_lens, grid_sizes,
                freqs)
        else:
            raise ValueError(f"Invalid mode '{mode}' specified for WanAttentionBlock")
        # with amp.autocast(dtype=torch.float32):
        x = x + y * e[2]
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            WanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads,
                              window_size, qk_norm, cross_attn_norm, context_mix, eps)
    ...
    def _forward(
        ...
        # print("seq", seq_len, "seq_lens.max()", seq_lens.max())
