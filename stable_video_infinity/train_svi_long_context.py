...
from diffsynth import SVIVideoContextPipeline, ModelManager, SVIVideoPipeline, load_state_dict, load_state_dict_from_folder
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="Probability threshold for context error in error settings (range 0-1).",
    )
    parser.add_argument(
        "--latent_prob",
        type=float,
...
class VideoDataset(Dataset):
    def __init__(self, dataset_paths, metadata_paths, target_fps=16, height=480, width=832):
        super().__init__()
        ...
        self.SINK_LEN = 9             
        self.TARGET_LEN = 84          
        self.MOTION_LEN = 24 # target and motion put together      
        self.MAX_CONTEXT_WINDOW = 18 * 16 # 18s video 
        self.CTX_CHUNK_STRIDE = 24    
        self.CTX_CHUNK_KEEP = 4       

        base_paths = dataset_paths.split(',')
        meta_paths = metadata_paths.split(',')
        assert len(base_paths) == len(meta_paths), "Paths mismatch."

        print(f"Dataset Init: FPS={target_fps}, Res={height}x{width}")
        print(f"Split Config: Min={self.MIN_FRAMES}, Sink={self.SINK_LEN}, "
              f"Target={self.TARGET_LEN}, Motion={self.MOTION_LEN}")

        for base_path, meta_path in zip(base_paths, meta_paths):
            base_path = base_path.strip()
            meta_path = meta_path.strip()
            if not os.path.exists(meta_path): continue
            try:
                try: df = pd.read_csv(meta_path)
                except: df = pd.read_csv(meta_path, sep='\t')
                
                path_col = 'videoFile' if 'videoFile' in df.columns else 'clip_id'
                if path_col not in df.columns: continue
                cap_col = 'caption' if 'caption' in df.columns else 'Detailed Description'

                valid_count = 0
                for _, row in df.iterrows():
                    rel_path = str(row[path_col])
                    full_path = os.path.join(base_path, rel_path)
                    if not os.path.exists(full_path) and not rel_path.endswith('.mp4'):
                        full_path += '.mp4'
                    if os.path.exists(full_path):
                        caption = row[cap_col] if cap_col in df.columns else ""
                        self.samples.append((full_path, caption))
                        valid_count += 1
                print(f" -> Added {valid_count} videos from {os.path.basename(base_path)}")
            except Exception as e:
    ...
    def process_video_resize(self, video_tensor):
        c, t, h, w = video_tensor.shape
        scale = max(self.height / h, self.width / w)
        new_h = int(round(h * scale))
        new_w = int(round(w * scale))
        video_tensor = F.resize(video_tensor, [new_h, new_w], interpolation=transforms.InterpolationMode.BICUBIC)
        video_tensor = F.center_crop(video_tensor, [self.height, self.width])
        return video_tensor

    def split_video_tensor(self, video_tensor):
        C, T, H, W = video_tensor.shape

        if T < self.MIN_FRAMES:
            raise ValueError(f"Video too short: {T} frames (min {self.MIN_FRAMES})")
        
        # A. Sink Frames: [0 : 9]
        sink_frames = video_tensor[:, :self.SINK_LEN, :, :]

        # B. Target Frames: [Last 84] -> [T-84 : T]
        target_start_idx = T - self.TARGET_LEN
        target_frames = video_tensor[:, target_start_idx:, :, :]

        # C. Motion Frames: [24 before Target] -> [T-108 : T-84], save one more for vae
        motion_end_idx = target_start_idx
        motion_start_idx = motion_end_idx - self.MOTION_LEN - 1 # save one more frame for vae
        motion_frames = video_tensor[:, motion_start_idx:motion_end_idx, :, :]

        # D. Context Frames Logic
        ctx_region_end = motion_start_idx 
        
        ctx_region_start_ideal = ctx_region_end - self.MAX_CONTEXT_WINDOW
        ctx_region_start = max(self.SINK_LEN, ctx_region_start_ideal)
        
        ctx_raw = video_tensor[:, ctx_region_start - 1:ctx_region_end, :, :] # (C, T_ctx, H, W)
        
        if ctx_raw.shape[1] == 0:
            raise ValueError("Context region empty (unexpected calculation error)")

        context_chunks = []
        ctx_len = ctx_raw.shape[1]
        
        for i in range(0, ctx_len, self.CTX_CHUNK_STRIDE):
            start = i
            end = min(i + self.CTX_CHUNK_KEEP, ctx_len) + 1
            
            if start < end:
                chunk_tensor = ctx_raw[:, start:end, :, :]
                context_chunks.append(chunk_tensor)
        
        if len(context_chunks) == 0:
             raise ValueError("Context sampling failed (no chunks generated)")

        return {
            "sink": sink_frames,       # (C, 9, H, W)
            "context": context_chunks, # List[Tensor], (C, 5, H, W)
            "motion": motion_frames,   # (C, 25, H, W)
            "target": target_frames    # (C, 84, H, W)
        }
    ...
    def __getitem__(self, index):
        while True:
            try:
                if index < 0: index = 0 
                video_path, caption = self.samples[index]
                
                vr = VideoReader(video_path, ctx=cpu(0), width=-1, height=-1, num_threads=1)
                total_frames = len(vr)
                original_fps = vr.get_avg_fps()
                
                if total_frames <= 0 or original_fps <= 0 or np.isnan(original_fps):
                    raise ValueError("Corrupt metadata")

                duration = total_frames / original_fps
                target_total_frames = int(duration * self.target_fps)
                
                if target_total_frames < self.MIN_FRAMES:
                    raise ValueError("Video too short")

                frame_indices = np.linspace(0, total_frames - 1, target_total_frames).astype(int)
                
                # 3. Decode -> Tensor
                video_data = vr.get_batch(frame_indices.tolist()).asnumpy()
                video_tensor = torch.from_numpy(video_data) # (T, H, W, C)
                video_tensor = video_tensor.permute(3, 0, 1, 2) # -> (C, T, H, W)
                video_tensor = (video_tensor / 127.5) - 1.0
                
                # 4. Resize & Crop
                video_tensor = self.process_video_resize(video_tensor)
                
                # 5. Split Logic 
                split_data = self.split_video_tensor(video_tensor)

                return {
                    "video_parts": split_data, 
                    "text": caption,
                    "fps": self.target_fps,
                    "path": video_path
                }
                ...
                # print(f"Error loading {self.samples[index][0]}: {e}")
...
class LightningModelForTrain_onestage(pl.LightningModule):
    def __init__(
        ...
        self.pipe = SVIVideoContextPipeline.from_model_manager(model_manager, is_test=False)
        ...
        self.context_error_buffer = {i: [] for i in range(num_grids)}
        ...
        self.context_prob = getattr(args, 'context_prob', 0.99)
    ...
    def _add_error_to_context_buffer(self, error_sample, timestep):
        """Add error sample to buffer using specified replacement strategy based on timestep grid."""
        grid_idx = self._get_timestep_grid(timestep)
        error_cpu = error_sample.detach().cpu()
        
        if len(self.context_error_buffer[grid_idx]) < self.error_buffer_size:
            # Buffer not full, simply add
            self.context_error_buffer[grid_idx].append(error_cpu)
        else:
            # Buffer full, use specified replacement strategy
            if self.buffer_replacement_strategy == "random":
                # Random replacement - O(1), fastest
                replace_idx = random.randint(0, len(self.context_error_buffer[grid_idx]) - 1)
                self.context_error_buffer[grid_idx][replace_idx] = error_cpu
                
            elif self.buffer_replacement_strategy == "fifo":
                # First-in-first-out - O(1), simple queue behavior
                self.context_error_buffer[grid_idx].pop(0)
                self.context_error_buffer[grid_idx].append(error_cpu)
                
            elif self.buffer_replacement_strategy == "l2_batch":
                # Batch L2 computation - O(n) but vectorized, much faster than original
                distances = self._compute_l2_distance_batch(error_cpu, self.context_error_buffer[grid_idx])
                most_similar_idx = torch.argmin(distances).item()
                self.context_error_buffer[grid_idx][most_similar_idx] = error_cpu
    ...
    def _sample_latent_error_from_latent_buffer(self, latents, timestep):
        ...
        if not self.context_error_buffer[grid_idx]:
            return torch.zeros_like(latents)
        
        # Randomly select one sample from the corresponding grid
        selected_sample = random.choice(self.context_error_buffer[grid_idx])
        error_sample = selected_sample.to(self.device)
    ...
    def _sample_context_latent_error_from_context_latent_buffer(self, latents, timestep):
        """Randomly sample an error from the buffer based on timestep grid."""
        all_samples = []
        for grid_idx, buffer in self.context_error_buffer.items():
    ...
    def training_step(self, batch, batch_idx):
        text, video_parts = batch["text"][0], batch["video_parts"]
        sink = video_parts["sink"]
        context = video_parts["context"]
        motion = video_parts["motion"]
        target = video_parts["target"]

        self.pipe_VAE.device = self.device
        
        with torch.no_grad():
            if video_parts is not None:
                # prompt
                ...
                # encode sink
                sink = sink.to(dtype=torch.float32, device=self.pipe_VAE.device)
                sink_latents = self.pipe_VAE.encode_video(sink, **self.tiler_kwargs)[0]
                sink_latents = sink_latents.to(dtype=original_dtype)

                # encode context
                context_latents_list = []
                for i in range(len(context)):
                    current_context = context[i].to(dtype=torch.float32, device=self.pipe_VAE.device)
                    current_context_latents = self.pipe_VAE.encode_video(current_context, **self.tiler_kwargs)[0]
                    current_context_latents = current_context_latents[:, 1:2, :, :]
                    current_context_latents = current_context_latents.to(dtype=original_dtype)
                    context_latents_list.append(current_context_latents)

                context_latents = torch.cat(context_latents_list, dim=1)
                
                # encode local motion and target
                motion_and_target = torch.cat([motion, target], dim=2)
                motion_and_target = motion_and_target.to(dtype=torch.float32, device=self.pipe_VAE.device)
                motion_and_target_latents = self.pipe_VAE.encode_video(motion_and_target, **self.tiler_kwargs)[0]
                motion_and_target_latents = motion_and_target_latents.to(dtype=original_dtype)

                motion_latents = motion_and_target_latents[:, 1:7, :, :]
                target_latents = motion_and_target_latents[:, 7:, :, :]
                
                latents = torch.cat([sink_latents, context_latents, motion_latents, target_latents], dim=1)
        ...
        latents = batch["latents"].to(self.device)  # [1, 16, 42, 60, 104]
        ...
        motion_latent_len = 6
        sink_latent_len = 6
        # context_latents = latents[:, :, :tgt_latent_len, ...] # [1, 16, 21, 60, 104]
        ...
        noise_w_error = noise[:, :, -tgt_latent_len:, ...]
        latents_w_error = latents[:, :, -tgt_latent_len:, ...]
        motion_w_error = latents[:, :, -(tgt_latent_len+motion_latent_len):-tgt_latent_len, ...]
        sink_latent = latents[:, :, :sink_latent_len, ...]
        context_latent = latents[:, :, sink_latent_len:-(tgt_latent_len+motion_latent_len), ...]
        ...
        has_context_latent_buffer_data = len(self.context_error_buffer[current_grid_idx]) > 0
        ...
        #     add_error_y = True
        if context_random < self.context_prob:
        ...
        if add_error_noise and has_latent_buffer_data:
            ...
            noise_w_error = noise[:, :, -tgt_latent_len:, ...] + noise_error_sampled.to(latents.dtype)
        ...
        if add_error_context and has_context_latent_buffer_data:
            context_latent_error_sampled = self._sample_context_latent_error_from_context_latent_buffer(motion_latents, timestep)
            max_start_idx = max(0, context_latent_error_sampled.shape[2] - motion_latent_len)
            
            # Use random selection (original behavior)
            random_frame_idx = torch.randint(0, max_start_idx + 1, (1,)).item()

            error_to_add = context_latent_error_sampled[:, :, random_frame_idx:random_frame_idx+motion_latent_len, ...]
            motion_w_error = motion_latents + error_to_add.to(motion_latents.dtype)
        ...
        if add_error_latent and has_latent_buffer_data:
            ...
            latents_w_error = latents[:, :, -tgt_latent_len:, ...] + latent_error_sampled.to(latents.dtype)
            
        extra_input = self.pipe.prepare_extra_input(latents)
        noisy_latents = self.pipe.scheduler.add_noise(latents_w_error, noise_w_error, timestep)  # error-corrupted X_t
        noisy_latents = torch.cat([sink_latent, context_latent, motion_w_error, noisy_latents], dim=2) # First part as context_w_error
        training_target = self.pipe.scheduler.training_target(latents[:, :, -tgt_latent_len:, ...], noise_w_error, timestep)  # self-corrected velocity pointing to clean latent
        ...
        print(noisy_latents.shape, noise_pred.shape)
        ...
        if self.use_error_recycling:
            with torch.no_grad():
                ...
                noise_corr_gt = self.pipe.scheduler.step(training_target, timestep, noisy_latents[:, :, -tgt_latent_len:, ...], to_final=True, self_corr=True)
                
                noise_error = x_0_pred[:, :, -tgt_latent_len:, ...] - noise_corr_gt
                x_1_pred = self.pipe.scheduler.step(noise_pred, timestep, noisy_latents, to_final=True, self_corr=False)
            
                latent_corr_gt = self.pipe.scheduler.step(training_target, timestep, noisy_latents[:, :, -tgt_latent_len:, ...], to_final=True, self_corr=False)
                y_error = x_1_pred[:, :, -tgt_latent_len:, ...] - latent_corr_gt

                is_warmup_phase = current_step <= self.buffer_warmup_iter
        ...
        if self.use_error_recycling:
            ...
            total_y_buffer_size = sum(len(buffer) for buffer in self.context_error_buffer.values())

            if self.global_step % 1 == 0 and self.global_rank == 0:  
                print(f"[REAL-TIME LOG] Step: {self.global_step} | "
                    f"Loss: {loss.item():.4f} | "
                    f"LatentBuf: {total_latent_buffer_size} | "
                    f"yBuf: {total_y_buffer_size}", flush=True)
            ...
            self.log(f"context_error_buffer_grid_{current_grid_idx}", len(self.context_error_buffer[current_grid_idx]), prog_bar=False)
    ...
    def _update_error_buffers_distributed(self, gathered_noise_errors, gathered_y_errors, gathered_timesteps):
        ...
        for i in range(num_gpus):
            ...
            self._add_error_to_context_buffer(y_error_sample, timestep_sample)
    ...
    def _update_error_buffers_local(self, noise_error, y_error, timestep):
        ...
        self._add_error_to_context_buffer(y_error, timestep)
