...
from diffsynth import ModelManager, WanVideoReCamMasterPipeline, save_video, VideoData, WanVideoPipeline
...
class TextVideoCameraDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, metadata_path, args, max_num_frames=81, frame_interval=1, num_frames=81, height=480, width=832, is_i2v=False):
        ...
        self.paths = [os.path.join(base_path, file_name) for file_name in metadata["clip_id"]]
        self.texts = metadata["Summarized Description"].to_list()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save the lora model.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        action='store_false', 
        dest='tiled', 
        help="Disable tiled decoding. By default, it's enabled."
    )
    parser.add_argument(
        '--num_chunks',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=81,
        help="Generate frame number."
    )
    parser.add_argument(
        '--from_scratch', 
        action='store_true',
        default=False,
        help="Generate from scratch, include first chunk."
    )
    parser.add_argument(
        "--train_architecture",
        default='lora',
...
if __name__ == '__main__':
    ...
    pipe = WanVideoReCamMasterPipeline.from_model_manager(model_manager, device="cuda", torch_dtype=torch.bfloat16)
        ...
        for chunk_id in range(args.num_chunks):
            if chunk_id == 0:
                ...
                if args.from_scratch:
                    ...
                    source_frames = pipe.tensor2video(source_video[0])
                    source_latents = None
                    # generated_frames += source_frames
            # target_camera = batch["camera"]
 
            print(f"Tiled:{args.tiled}")
            video, all_video = pipe(
                prompt=target_text,
            ...
            #     seed=0, tiled=args.tiled, 
            # )
            source_video = all_video[:,:,-args.num_frames:,...]
            # Save source video at first chunk
            # generated_frames += video
            if chunk_id > 0:
                generated_frames += video
            else:
                generated_frames += pipe.tensor2video(all_video[0])
        save_video(generated_frames, os.path.join(args.output_dir, f"video_{batch_idx}_{target_text[0][:50]}.mp4"), fps=16, quality=5)