...
class SVIVideoContextPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False, is_test=False, num_train_timesteps=1000):
        ...
        pipe = SVIVideoContextPipeline(device=device, torch_dtype=torch_dtype, is_test=is_test, num_train_timesteps=num_train_timesteps)
    ...
    def __call__(
        ...
        input_image=None,
        input_video=None,
        source_video=None, # Added source_video argument 
        ...
        # Latent Initialization / Encoding
        # Scenario 1: T2V (source_video is None)
        # Scenario 2: Context Generation (source_video is provided)
        ...
        if source_video is not None:
             # Encode sink, context, motion
             self.load_models_to_device(['vae'])
             
             # Extract components from source_video
             # They are expected to be tensors (C, T, H, W) on CPU or GPU
             sink = source_video.get("sink")
             context_chunks = source_video.get("context", [])
             motion = source_video.get("motion")
             
             original_dtype = self.vae.torch_dtype
             self.vae.to(torch.float32)
             
             # Encode Sink
             if sink is not None:
                 sink = sink.to(device=self.device, dtype=torch.float32).unsqueeze(0) # (1, C, T, H, W)
                 sink_latents = self.encode_video(sink, **tiler_kwargs) # (1, 16, T_latent, H_latent, W_latent)
                 sink_latent = sink_latents
                 
             # Encode Context Chunks
             context_latents_list = []
             for chunk in context_chunks:
                 chunk = chunk.to(device=self.device, dtype=torch.float32).unsqueeze(0)
                 # Training script usually takes [1:2] of latent time dim for context? 
                 # Train logic: current_context_latents = self.pipe_VAE.encode_video(...)[0]
                 #              current_context_latents = current_context_latents[:, 1:2, :, :]
                 chunk_latents = self.encode_video(chunk, **tiler_kwargs)
                 if chunk_latents.shape[2] > 1:
                     chunk_latents = chunk_latents[:, :, 1:2, :, :]
                 context_latents_list.append(chunk_latents)
                 
             if context_latents_list:
                 context_latent = torch.cat(context_latents_list, dim=2)
                 
             # Encode Motion
             if motion is not None:
                 motion = motion.to(device=self.device, dtype=torch.float32).unsqueeze(0)
                 motion_latents_full = self.encode_video(motion, **tiler_kwargs)
                 motion_latent = motion_latents_full
                 if motion_latent.shape[2] >= 7:
                      motion_latent = motion_latent[:, :, 1:7, :, :] # Slice to match training 6 latents if needed
                 elif motion_latent.shape[2] > 0:
                      pass 
                      
             self.vae.to(original_dtype)
             
             parts = []
             if sink_latent is not None: parts.append(sink_latent)
             if context_latent is not None: parts.append(context_latent)
             if motion_latent is not None: parts.append(motion_latent)
             parts.append(noise)
             
             latents = torch.cat(parts, dim=2)
             
        elif input_video is not None:
            self.load_models_to_device(['vae'])
            input_video = self.preprocess_images(input_video)
            input_video = torch.stack(input_video, dim=2).to(dtype=self.torch_dtype, device=self.device)
            latents_input = self.encode_video(input_video, **tiler_kwargs).to(dtype=self.torch_dtype, device=self.device)
            latents = self.scheduler.add_noise(latents_input, noise, timestep=self.scheduler.timesteps[0])
        else:
            latents = noise
        ...
        # Fix cfg_scale
        if not isinstance(cfg_scale, dict):
            cfg_scale = {'text': cfg_scale}
        ...
        # For long context, we only want to decode the TARGET part (the last part corresponding to noise)
        target_len = noise.shape[2]
        target_latents = latents[:, :, -target_len:, :, :]
        
        # Decode
        self.load_models_to_device(['vae'])
        frames = self.decode_video(target_latents, **tiler_kwargs)
